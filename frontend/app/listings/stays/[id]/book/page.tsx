...
function StayBookingPageContent({ params }: Props) {
  ...
  // Cutign the safety timeout for the mpesa and cancellation of the payment
  ...
  // If the browser restores this page from bfcache (e.g. tapping back after
  // a completed booking), force a real reload instead of showing the frozen,
        ...
        callback: (response: { reference: string }) => {
          apiFetch<{ booking: { id: number } }>('/bookings/verify-and-create', {
            ...
            .then(async ({ booking }) => {
              ...
              router.replace(`/listings/stays/${id}/book/success?booking=${booking.id}`)
            })
    ...
    try {
      ...
      const poll = async (): Promise<void> => {
        ...
        if (result.status === 'success') {
          ...
          router.replace(`/listings/stays/${id}/book/success?booking=${result.booking_id}`)
          return
        }
          ...
          <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center"
            ...
            <div className="bg-white w-full sm:max-w-sm rounded-t-3xl sm:rounded-2xl p-6">
              ...
              <div className="flex gap-2">
                <button onClick={() => router.push(`/signup?redirect=${encodeURIComponent(`/listings/stays/${id}/book${searchParams.toString() ? '?' + searchParams.toString() : ''}`)}`)}
