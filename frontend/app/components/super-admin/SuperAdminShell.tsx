...
const NAV_ITEMS = [
  ...
  { label: 'Admins',      Icon: Shield,           path: '/super-admin/admins' },
  { label: 'Config',      Icon: Settings,         path: '/super-admin/config' },
  { label: 'Payments',    Icon: CreditCard,       path: '/super-admin/payments' },
  { label: 'Moderation',  Icon: ShieldAlert,      path: '/super-admin/moderation' },
    ...
    <div className="flex h-screen bg-white overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-64 bg-[#161616] flex-shrink-0">
        <div className="px-6 py-5 border-b border-white/10">
          ...
          <p className="text-white/40 text-xs mt-0.5">Super Admin</p>
        </div>
        ...
        <div className="px-3 py-4 border-t border-white/10">
          <div className="flex items-center gap-3 px-3 py-2 mb-2">
            <div className="w-8 h-8 rounded-full bg-white flex items-center
                            justify-center text-[#161616] text-sm font-bold flex-shrink-0">
              {user?.name?.[0]?.toUpperCase() ?? 'S'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-white text-sm font-semibold truncate">{user?.name ?? 'Super Admin'}</p>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50 flex">
          <div className="flex flex-col w-72 bg-[#161616] h-full shadow-2xl">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white">
        <div className="lg:hidden flex items-center justify-between px-4 py-3
          ...
          <div className="w-9 h-9 rounded-full bg-white flex items-center
                          justify-center text-[#161616] text-sm font-bold">
            {user?.name?.[0]?.toUpperCase() ?? 'S'}
          </div>
        </div>
